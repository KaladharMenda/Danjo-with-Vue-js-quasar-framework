...
def get_attendence_names(request):
    ...
    if attendence_dict['year_sem'] :
        existing_pin = []
        attendence_obj = Attendence_details.objects.filter(year_sem = attendence_dict['year_sem'],period =attendence_dict['period'] ,month = attendence_dict['month'],student_details__status = True).order_by('student_details__pin')
...
def all_attendence(request):
    ...
    try:
        if attendence_dict['period'] == 'complete':
            attendence_obj = Attendence_details.objects.filter(year_sem = attendence_dict['year_sem'],month = attendence_dict['month'],student_details__status = True).order_by('student_details__pin')
        else:
            attendence_obj = Attendence_details.objects.filter(year_sem = attendence_dict['year_sem'],period =attendence_dict['period'] ,month = attendence_dict['month'],student_details__status = True).order_by('student_details__pin')
        if attendence_obj.exists() :
...
def get_unit_marks(request):
    ...
    existing_pin = []
    unit_obj = Unit_marks.objects.filter(year_sem = unit_dict['year_sem'],subject =unit_dict['subject'],unit_exam = unit_dict['unit_exam'],student_details__status = True).order_by('student_details__pin')
    if unit_obj.exists() :
...
def get_pm_marks(request):
    ...
    if pm_dict.get('view','') :
        view = True
        pm_obj = Project_marks.objects.filter(year_sem = pm_dict['year_sem'],scheme_code =pm_dict['scheme_code'],student_details__status = True).order_by('student_details__pin')
    else:
        pm_obj = Project_marks.objects.filter(year_sem = pm_dict['year_sem'],scheme_code =pm_dict['scheme_code'],project_title = pm_dict['project_title'],student_details__status = True).order_by('student_details__pin')
    if pm_obj.exists() :
        for pm in pm_obj:
            student_dict ={}
